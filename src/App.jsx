...
function TechnologyPage({ nav }) {
  ...
  return <>
    ...
    <section className="sec-pad" style={{ maxWidth: 1200, margin: "0 auto", padding: "100px 40px", textAlign: "center" }}><Reveal><h2 className="sec-title" style={{ fontFamily: "'Playfair Display', serif", fontSize: 36, fontWeight: 700, marginBottom: 16 }}>See GoldOS in Action</h2><p style={{ color: C.muted, fontSize: 16, marginBottom: 32, maxWidth: 480, margin: "0 auto 32px" }}>We'll walk you through the platform and discuss what it means for your operation.</p><Btn primary onClick={() => nav("contact")}>Request a Demo →</Btn></Reveal></section>
  </>;
}
...
export default function App() {
  ...
  return <div style={{ background: C.dark, color: C.white, fontFamily: "'DM Sans', sans-serif", minHeight: "100vh", overflowX: "hidden" }}>
    ...
    <nav className="nav-outer" style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, background: scrolled ? "rgba(10,14,23,0.95)" : "rgba(10,14,23,0.8)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(255,255,255,0.05)", transition: "all 0.4s", padding: "0 40px" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center", height: 64 }}>
        ...
        <button className="nm" onClick={() => setMm(!mm)} style={{ display: "none", background: "none", border: "none", color: C.white, fontSize: 22, cursor: "pointer", padding: "4px 8px" }}>☰</button>
      </div>
    ...
    {page !== "contact" && <section style={{ background: `linear-gradient(135deg, rgba(201,162,39,0.1), rgba(201,162,39,0.03))`, borderTop: "1px solid rgba(201,162,39,0.15)" }}><div className="cta-inner" style={{ maxWidth: 1200, margin: "0 auto", padding: "64px 40px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 24 }}><div><h3 style={{ fontSize: 24, fontWeight: 700, marginBottom: 4 }}>Ready to Raise the Standard?</h3><p style={{ color: C.muted, fontSize: 15 }}>Let's talk about what Gold can do for your facility or your career.</p></div><Btn primary onClick={() => nav("contact")}>Schedule a Conversation →</Btn></div></section>}

    <footer style={{ borderTop: "1px solid rgba(255,255,255,0.05)", background: C.navy }}><div className="footer-inner" style={{ maxWidth: 1200, margin: "0 auto", padding: "56px 40px 32px" }}><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(180px, 100%), 1fr))", gap: 40, marginBottom: 40 }}>
